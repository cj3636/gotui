...
- **Weather** – Current conditions from [wttr.in](https://wttr.in). Override location with `WTTR_LOCATION` and append custom query strings (e.g., `format=3&M` for metric units) via `WTTR_PARAMS`.
- **Moon Phase** – wttr.in `/moon` view with customizable location (`WTTR_MOON_LOCATION`) and query parameters (`WTTR_MOON_PARAMS`, default `format=Moon:+%m`).
...
| --- | --- | --- |
| `WTTR_LOCATION` | Weather location/endpoint segment passed to wttr.in. | `San Francisco` |
| `WTTR_PARAMS` | Additional wttr.in query string for weather (e.g., `format=3&M`). | `format=3` |
| `WTTR_MOON_LOCATION` | wttr.in moon endpoint segment (typically `moon`). | `moon` |
| `WTTR_MOON_PARAMS` | Query string for moon data (e.g., `format=Moon:+%m`). | `format=Moon:+%m` |
